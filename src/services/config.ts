...
export const TMDB_CREDENTIAL = readEnv('VITE_TMDB_API_KEY');
/** @deprecated Prefer TMDB_CREDENTIAL — alias kept for existing imports */
export const API_KEY = TMDB_CREDENTIAL;
export const APP_BUILD = readEnv('VITE_APP_BUILD') || 'dev';

export type TmdbAuthMode = 'api_key' | 'bearer';

/** Strips accidental "Bearer " prefix from .env values. */
export function normalizeTmdbCredential(raw: string): string {
...
/**
 * TMDB v3 API Key (~32 alfanum) → query ?api_key=
 * TMDB v4 Read Access Token (JWT eyJ...) → header Authorization: Bearer
 */
export function getTmdbAuthMode(credential: string): TmdbAuthMode {
  const token = normalizeTmdbCredential(credential);
  if (!token) return 'api_key';
  if (token.startsWith('eyJ')) return 'bearer';
  return 'api_key';
}

/** Applies the correct auth scheme to the request URL and returns fetch headers. */
export function buildTmdbAuth(
  url: URL,
  credential: string = TMDB_CREDENTIAL,
): Headers {
  const token = normalizeTmdbCredential(credential);
  ...
  if (getTmdbAuthMode(token) === 'bearer') {
